...
import com.cgg.twdinspection.gcc.adapter.PLPGAdapter;
...
public class PLPGFragment extends Fragment {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            String petComm = getArguments().getString(AppConstants.petComm);
            Type listType = new TypeToken<ArrayList<CommonCommodity>>() {
            }.getType();
            commonCommodities = new Gson().fromJson(petComm, listType);
        }
    }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        PLPGAdapter plpgAdapter = new PLPGAdapter(getActivity(), commonCommodities);
        binding.groupRV.setLayoutManager(new LinearLayoutManager(getActivity()));
        binding.groupRV.setAdapter(plpgAdapter);
